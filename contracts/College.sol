...
constructor() public {
   ...
   //Task 1 : call the owner of the contractor and we are only giving whole access to the address who deplyed the contract
    ...
    require(msg.sender == minter); // minter missed(address)
    ...
    uint xy = cLass;
      ...
      if (CLASS < 8) {         
        ...
        else if (CLASS == 10) {// 10 value missed
        totalfees = 70000;
        }
         else if (CLASS> 10) {
         totalfees = 0; //totalfees variable missed
        }
       uint d = cAste;
      uint CASTE = d/1;
      ...
      //If CASTE is equal to 4 then castescholarship has to be 4000
         ...
         if (CASTE == 1) {
        castescholarship =  1000; //1000 missed
        }
       else if (CASTE == 2) {
        castescholarship = 2000; //castescholarship variable missed
        }
        
        else if (CASTE == 3) { //3 value missed
        castescholarship = 3000;
        }
        
       
        else if (CASTE == 4) {// CASTE is missed
